...
function vaiCategoria(category) {
	window.location.href = `categorie.html?categoria=${category}`;
}

window.addEventListener("load", function () {
	if (window.location.href.includes("categorie.html")) {
	...
	fetch(`https://fakestoreapi.com/products/${id}`)
		...
		.then((data) => {
			...
			setTimeout(function () {
				...
				document.getElementById("containerLoading").style.height = "0px";
		...
		.then((data) => {
			...
			setTimeout(function () {
				...
				for (let i = 0; i < prodotti.length; i++) {
					...
					divProdotto.classList.add("cardCategorie");
					...
					divProdotto.innerHTML = `<img style="cursor:pointer" onclick = vaiAlProdotto(${prodotto.id}) src="${immagine}" alt="" id="imageCategorie">
                        <div class="infoCategorie">
					...
					document.getElementById("categorie").appendChild(divProdotto);
